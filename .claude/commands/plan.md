...
In PRD artifact mode, create `.claude/plans/` if needed. If the PRD contains a `Delivery Milestones` table, update only the selected row from `pending` to `in-progress` and set its `Plan` cell to the generated plan path.
...
This repository ships a `planner` agent (`.claude/agents/planner.md`). Delegate
to it for non-trivial features when the runtime exposes subagents; otherwise run
this command inline.
